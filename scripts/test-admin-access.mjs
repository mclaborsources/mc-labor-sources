...
async function invoke(body, options = {}) {
  ...
  const admin = {
    from: () => ({
      select: () => ({
        eq: () => ({ single: async () => ({ data: { role: options.role || 'ADMIN', status: options.status || 'ACTIVE' } }), maybeSingle: async () => ({ data: options.detached ? null : { id: 'profile', role: options.targetRole || 'ADMIN' } }) }),
  ...
  vm.runInNewContext(source, {
    Response, Deno: { env: { get: () => 'test' }, serve: fn => { handler = fn; } },
