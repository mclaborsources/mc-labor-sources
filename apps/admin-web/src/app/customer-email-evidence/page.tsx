...
export default function CustomerEmailEvidencePage() {
  ...
  const selectedBatchId = originalBatches.some((batch) => batch.id === batchId)
    ? batchId : originalBatches[0]?.id ?? '';
  const selectedBatch = originalBatches.find((batch) => batch.id === selectedBatchId);
  ...
  function exportRecordJson() {
    ...
    download(`timesheet-email-record-${selectedBatch.id}.json`, JSON.stringify({
      ...
      emailChain: chainBatches,
      timesheets: batchItems,
      portalDecisions: relatedDecisions,
      importedEmailCopies: batchEvidence.data.imports,
      note: 'This record starts with the first sent email and includes its timesheets and subsequent recorded activity. Actions without source_batch_id are historical and cannot be attributed to a specific email. Imported copies are user supplied.',
  ...
  return <DashboardLayout heroTitle="Customer Email Evidence" heroImage={BRAND_HERO_IMAGES.inner}>
    ...
    <div className="space-y-5 rounded-xl border border-slate-200 bg-white p-5">
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="font-semibold">Customer<select className="mt-2 w-full rounded-lg border p-2" value={customerId} onChange={(event) => { setCustomerId(event.target.value); setBatchId(''); setUploadBatchId(''); }}><option value="">Choose a customer</option>{customers.data?.map((item) => <option key={item.id} value={item.id}>{item.company_name}</option>)}</select></label>
        <label className="font-semibold">First sent email<select className="mt-2 w-full rounded-lg border p-2" value={selectedBatchId} onChange={(event) => { setBatchId(event.target.value); setUploadBatchId(''); }} disabled={!originalBatches.length}><option value="">Choose the first email</option>{originalBatches.map((batch) => { const count = batchItemCount(batch.id); return <option key={batch.id} value={batch.id}>{new Date(batch.sent_at).toLocaleString()} · {count} timesheet{count === 1 ? '' : 's'} · {batch.subject}</option>; })}</select></label>
      </div>
      ...
      {selectedBatch ? <>
        <div className="rounded-lg border border-blue-200 bg-blue-50 p-4">
          ...
          <p className="mt-2 text-sm text-slate-700">This email included <strong>{batchItems.length} timesheet{batchItems.length === 1 ? '' : 's'}</strong>. The record below follows those timesheets through {chainBatches.length - 1} follow-up email{chainBatches.length - 1 === 1 ? '' : 's'} and all recorded customer actions.</p>
        </div>
        <div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-xl font-bold">Complete record from this first email</h2><div className="flex flex-wrap gap-2"><button type="button" onClick={() => void exportEvidence()} disabled={batchEvidence.isPending || Boolean(batchEvidence.error) || pdfExporting} className="rounded-lg bg-blue-700 px-4 py-2 font-bold text-white disabled:opacity-50">{pdfExporting ? 'Preparing PDF…' : 'Download full email record (PDF)'}</button><button type="button" onClick={exportRecordJson} disabled={batchEvidence.isPending || Boolean(batchEvidence.error)} className="rounded-lg border border-blue-700 px-4 py-2 font-bold text-blue-700 disabled:opacity-50">Download complete data</button></div></div>
        ...
        <p className="text-sm text-slate-600">The timeline begins with the selected first email and includes follow-up requests, approvals, requested changes, and imported email copies for all timesheets that were in that first email.</p>
        {chainBatches.some((batch) => batch.sent_raw_base64) ? <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4"><h2 className="font-bold">Sent emails with PDF attachments</h2><div className="mt-2 flex flex-wrap gap-2">{chainBatches.filter((batch) => batch.sent_raw_base64).map((batch) => <button key={batch.id} type="button" className="rounded-lg border border-emerald-500 bg-white px-3 py-2 text-sm font-semibold text-emerald-800" onClick={() => downloadEmail(`sent-timesheets-${batch.id}.eml`, batch.sent_raw_base64!)}>Download request {batch.request_number ?? 1} · {new Date(batch.sent_at).toLocaleString()}</button>)}</div></div> : null}
        ...
        <div className="space-y-3 rounded-lg border border-blue-200 bg-blue-50 p-4"><h2 className="font-bold">Add an original email copy</h2><p className="text-sm">Export a sent message or customer reply as an .eml file from your mailbox, then attach it to the matching email in this chain. Imported copies appear in the timeline.</p><select aria-label="Related sent email" className="w-full rounded-lg border p-2" value={uploadBatchId} onChange={(event) => setUploadBatchId(event.target.value)}><option value="">Choose an email in this chain</option>{chainBatches.map((batch) => <option key={batch.id} value={batch.id}>Request {batch.request_number ?? 1} · {new Date(batch.sent_at).toLocaleString()} · {batch.subject}</option>)}</select><input type="file" accept=".eml,message/rfc822" aria-label="Original .eml file" onChange={(event) => setUploadFile(event.target.files?.[0] ?? null)} /><button type="button" disabled={!uploadBatchId || !uploadFile || uploading} onClick={() => void importReply()} className="rounded-lg bg-blue-700 px-4 py-2 font-bold text-white disabled:opacity-50">{uploading ? 'Importing…' : 'Import email copy'}</button>{uploadError ? <p role="alert" className="text-red-700">{uploadError}</p> : null}</div>
      </> : null}
    </div>
  </DashboardLayout>;
}
