...
export function CustomerTimesheetApproval({ token, approveAll = false, demo = false }: { token: string; approveAll?: boolean; demo?: boolean }) {
  ...
  const { panelRef, popupStyle } = usePopupPosition(Boolean(reviewingId), true);
  const [comments, setComments] = useState<Record<string, string>>({});
  const [approvedAll, setApprovedAll] = useState(false);
  const load = useCallback(async () => { if (demo) { setBatch(DEMO_BATCH); setLoading(false); return; } if (!token) { setError('This approval link is missing its secure token.'); setLoading(false); return; } try { const loaded = await requestPage(token, approveAll ? 'approve_all' : 'load'); setBatch(loaded); setApprovedAll(approveAll); } catch (e) { setError(e instanceof Error ? e.message : 'Could not load this approval request.'); } finally { setLoading(false); } }, [approveAll, demo, token]);
  ...
  const reviewComplete = Boolean(batch?.timesheets.length && batch.timesheets.every(item => item.approvedAt || item.reviewRequestedAt));
  ...
  return <main className="min-h-screen bg-slate-100 text-slate-900 sm:px-6 sm:py-8 lg:px-8"><div className="mx-auto max-w-[1600px] overflow-hidden bg-white shadow-xl sm:rounded-2xl sm:border sm:border-slate-200">
    <header className="border-b border-slate-200 bg-slate-950 px-5 py-5 text-white sm:px-10 sm:py-8"><p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-300 sm:text-base">MC Labor Sources</p><div className="mt-1 flex items-end justify-between gap-3 sm:block"><div><h1 className="text-2xl font-black sm:mt-2 sm:text-4xl">Review Timesheets</h1>{batch ? <p className="mt-1 text-base text-slate-300 sm:mt-2 sm:text-lg">{batch.customerName}</p> : null}</div>{demo ? <span className="mb-1 rounded-full bg-blue-400/15 px-2.5 py-1 text-xs font-bold uppercase tracking-wider text-blue-200">Demo</span> : null}</div></header>
    <div className="p-4 sm:p-10">{loading ? <p className="py-16 text-center text-base text-slate-500">{approveAll ? 'Approving all timesheets…' : 'Loading timesheets…'}</p> : null}{error ? <p className="mb-5 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-base font-semibold text-red-700">{error}</p> : null}
      {approvedAll && batch ? <p className="mb-5 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-5 text-base font-bold text-emerald-800">All eligible timesheets attached to this email have been approved. Thank you.</p> : null}
      {reviewComplete && !loading && !busyId ? <p role="status" className="mb-5 rounded-xl border border-emerald-700 bg-emerald-600 px-4 py-5 text-lg font-bold text-white sm:text-xl">Thank you for approving the hours. If you have any additional questions, please contact our office at 1-800-439-3360 or reply to this email.</p> : null}
      {batch ? <><div className="mb-3 flex items-center justify-between gap-3 rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-base text-blue-950 sm:mb-6 sm:rounded-xl sm:px-5 sm:py-4 sm:text-lg"><span><strong>{batch.timesheets.length}</strong> to review</span><span>{batch.timesheets.filter((i) => i.approvedAt).length} approved · {batch.timesheets.filter((i) => i.reviewRequestedAt).length} changes</span></div>
      ...
      <div className="hidden overflow-x-auto rounded-xl border border-slate-300 [touch-action:pan-x] sm:block">
        <table className="w-full min-w-[1320px] border-collapse text-sm">
          <thead className="bg-black text-white"><tr><th colSpan={14} className="px-4 py-4 text-center text-2xl font-black">Mc Labor Sources, Inc. Timesheets</th></tr><tr>
            <th className="w-28 border border-white/50 px-3 py-3 text-left">Job Name</th><th className="w-28 border border-white/50 px-3 py-3 text-left">First Name</th><th className="w-36 border border-white/50 px-3 py-3 text-left">Last Name</th>
            {WEEK_DAYS.map((day, index) => { const date = dateAt(displayedTimesheets[0]?.weekStartDate ?? displayedTimesheets[0]?.workDate, index); return <th key={day} className="border border-white/50 px-3 py-3 text-center"><span className="block">{day}</span><span className="block text-xs">{shortDate(date)}</span></th>; })}
            <th className="border border-white/50 px-3 py-3 text-center">TH</th><th className="border border-white/50 px-3 py-3 text-center">RH</th><th className="border border-white/50 px-3 py-3 text-center">OT</th><th className="border border-white/50 px-3 py-3 text-left">Signed Timesheet / Action</th>
            ...
            <td className="whitespace-nowrap border border-slate-300 px-3 py-3 font-bold">{item.jobSiteName}</td><td className="whitespace-nowrap border border-slate-300 px-3 py-3">{item.employeeFirstName}</td><td className="whitespace-nowrap border border-slate-300 px-3 py-3">{item.employeeLastName}</td>
            {WEEK_DAYS.map((day, index) => <td key={day} className="border border-slate-300 px-3 py-3 text-center text-base font-semibold tabular-nums">{displayHours(hoursForDate(item, dateAt(start, index)))}</td>)}
            <td className="border border-slate-300 px-3 py-3 text-center text-base font-black tabular-nums">{displayHours(item.totalHours)}</td><td className="border border-slate-300 px-3 py-3 text-center text-base font-semibold tabular-nums">{displayHours(regular)}</td><td className="border border-slate-300 px-3 py-3 text-center text-base font-semibold tabular-nums">{displayHours(overtime)}</td>
            <td className="border border-slate-300 px-3 py-3"><p className={item.hasSignedTimesheet ? 'font-bold text-slate-700' : 'font-bold text-red-600'}>{item.hasSignedTimesheet ? 'Signed Timesheet attached' : 'No Timesheet'}</p><div className="mt-3 flex gap-2">{item.approvedAt ? <span className="text-base font-black text-emerald-700">✓ Approved</span> : item.reviewRequestedAt ? <span className="text-base font-black text-amber-800">Changes Requested</span> : <><button disabled={Boolean(busyId)} onClick={() => void decide(item.id, 'approve')} className="min-h-11 rounded bg-emerald-600 px-4 text-sm font-black text-white disabled:opacity-60">Approve</button><button disabled={Boolean(busyId)} onClick={() => setReviewingId(item.id)} className="min-h-11 rounded border border-amber-400 bg-amber-50 px-4 text-sm font-black text-amber-800 disabled:opacity-60">Request Changes</button></>}</div></td>
          </tr>; })}</tbody>
        </table>
      </div>
</> : null}
    </div></div>
    {reviewingId ? <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4" role="dialog" aria-modal="true" aria-labelledby="review-modal-title" onMouseDown={(event) => { if (event.target === event.currentTarget && !busyId) setReviewingId(''); }}><div ref={panelRef} style={popupStyle} className="w-full max-w-3xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl"><div className="border-b border-slate-200 px-9 py-8"><h2 id="review-modal-title" className="text-4xl font-black text-slate-950">Request Timesheet Changes</h2><p className="mt-3 text-xl text-slate-500">Tell MC Labor Sources what should be checked or corrected.</p></div><div className="p-9"><label className="text-xl font-bold text-slate-900">Change request <span className="font-normal text-slate-500">(optional)</span></label><textarea autoFocus rows={7} maxLength={2000} value={comments[reviewingId] ?? ''} onChange={(event) => setComments((current) => ({ ...current, [reviewingId]: event.target.value }))} placeholder="Describe what should be checked or corrected" className="mt-4 w-full resize-y rounded-lg border border-slate-300 bg-white px-5 py-4 text-xl outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-300"/><div className="mt-8 flex flex-wrap justify-end gap-4"><button type="button" disabled={Boolean(busyId)} onClick={() => setReviewingId('')} className="min-h-14 rounded-lg border border-slate-300 bg-white px-8 py-4 text-xl font-bold text-slate-700 hover:bg-slate-50 disabled:opacity-60">Cancel</button><button type="button" disabled={Boolean(busyId)} onClick={() => void decide(reviewingId, 'request_review')} className="min-h-14 rounded-lg bg-amber-600 px-8 py-4 text-xl font-bold text-white hover:bg-amber-700 disabled:opacity-60">{busyId ? 'Submitting…' : 'Confirm Request Changes'}</button></div></div></div></div> : null}
  </main>;
}
