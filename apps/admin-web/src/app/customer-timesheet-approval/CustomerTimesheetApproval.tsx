...
export function CustomerTimesheetApproval({ token, approveAll = false, demo = false }: { token: string; approveAll?: boolean; demo?: boolean }) {
  ...
  const { panelRef, popupStyle } = usePopupPosition(Boolean(reviewingId));
  ...
  return <main className="min-h-screen bg-slate-100 text-slate-900 sm:px-6 sm:py-8 lg:px-8"><div className="mx-auto max-w-[1600px] overflow-hidden bg-white shadow-xl sm:rounded-2xl sm:border sm:border-slate-200">
    ...
    {reviewingId ? <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4" role="dialog" aria-modal="true" aria-labelledby="review-modal-title" onMouseDown={(event) => { if (event.target === event.currentTarget && !busyId) setReviewingId(''); }}><div ref={panelRef} style={popupStyle} className="w-full max-w-2xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl"><div className="border-b border-slate-200 px-8 py-7"><h2 id="review-modal-title" className="text-3xl font-black text-slate-950">Request Timesheet Changes</h2><p className="mt-3 text-lg text-slate-500">Tell MC Labor Sources what should be checked or corrected.</p></div><div className="p-8"><label className="text-lg font-bold text-slate-900">Change request <span className="font-normal text-slate-500">(optional)</span></label><textarea autoFocus rows={6} maxLength={2000} value={comments[reviewingId] ?? ''} onChange={(event) => setComments((current) => ({ ...current, [reviewingId]: event.target.value }))} placeholder="Describe what should be checked or corrected" className="mt-4 w-full resize-y rounded-lg border border-slate-300 bg-white px-5 py-4 text-lg outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-300"/><div className="mt-7 flex flex-wrap justify-end gap-4"><button type="button" disabled={Boolean(busyId)} onClick={() => setReviewingId('')} className="min-h-13 rounded-lg border border-slate-300 bg-white px-7 py-3.5 text-lg font-bold text-slate-700 hover:bg-slate-50 disabled:opacity-60">Cancel</button><button type="button" disabled={Boolean(busyId)} onClick={() => void decide(reviewingId, 'request_review')} className="min-h-13 rounded-lg bg-amber-600 px-7 py-3.5 text-lg font-bold text-white hover:bg-amber-700 disabled:opacity-60">{busyId ? 'Submitting…' : 'Confirm Request Changes'}</button></div></div></div></div> : null}
  </main>;
}
