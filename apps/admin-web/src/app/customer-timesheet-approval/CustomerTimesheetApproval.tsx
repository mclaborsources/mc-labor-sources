...
export function CustomerTimesheetApproval({ token, selectedTimesheetId = '', intent, demo = false }: { token: string; selectedTimesheetId?: string; intent?: 'approve' | 'dispute'; demo?: boolean }) {
  ...
  const load = useCallback(async () => { if (demo) { setBatch(DEMO_BATCH); setCustomerNote(DEMO_BATCH.customerNote ?? ''); setLoading(false); return; } if (!token) { setError('This approval link is missing its secure token.'); setLoading(false); return; } try { const loaded = await requestPage(token, 'load'); setBatch(loaded); setCustomerNote(loaded.customerNote ?? ''); } catch (e) { setError(e instanceof Error ? e.message : 'Could not load this approval request.'); } finally { setLoading(false); } }, [demo, token]);
  ...
  const displayedTimesheets = batch
    ? [...batch.timesheets].filter((item) => !selectedTimesheetId || item.id === selectedTimesheetId).sort(
  ...
  return <main className="min-h-screen bg-slate-100 text-slate-900 sm:px-6 sm:py-8 lg:px-8"><div className="mx-auto max-w-[1600px] overflow-hidden bg-white shadow-xl sm:rounded-2xl sm:border sm:border-slate-200">
    ...
    <div className="p-3 sm:p-8">{loading ? <p className="py-16 text-center text-slate-500">Loading timesheet…</p> : null}{error ? <p className="mb-5 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">{error}</p> : null}
      {batch && selectedTimesheetId && displayedTimesheets.length === 0 ? <p className="mb-5 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">This employee's timesheet was not found in this email.</p> : null}
      {batch ? <>
      {intent && displayedTimesheets[0] && !displayedTimesheets[0].approvedAt && !displayedTimesheets[0].reviewRequestedAt ? <section className={`mb-4 rounded-xl border p-4 ${intent === 'approve' ? 'border-emerald-200 bg-emerald-50' : 'border-amber-200 bg-amber-50'}`}><h2 className="font-black text-slate-950">{intent === 'approve' ? `Approve ${displayedTimesheets[0].employeeName}'s timesheet?` : `Dispute ${displayedTimesheets[0].employeeName}'s timesheet?`}</h2><p className="mt-1 text-sm text-slate-700">{intent === 'approve' ? 'Confirming marks only this employee’s submitted hours as approved.' : 'A dispute does not approve the hours. It returns this timesheet to MC Labor Sources for correction and removes it from the ready-to-send queue.'}</p>{intent === 'dispute' ? <label className="mt-3 block text-sm font-bold text-slate-900">Reason for dispute <span className="font-normal text-slate-500">(optional)</span><textarea autoFocus rows={4} maxLength={2000} value={comments[displayedTimesheets[0].id] ?? ''} onChange={(event) => setComments((current) => ({ ...current, [displayedTimesheets[0].id]: event.target.value }))} placeholder="Describe what should be checked or corrected" className="mt-2 w-full resize-y rounded-lg border border-amber-300 bg-white px-3 py-2 text-sm font-normal outline-none focus:ring-2 focus:ring-amber-300" /></label> : null}<button type="button" disabled={Boolean(busyId)} onClick={() => void decide(displayedTimesheets[0].id, intent === 'approve' ? 'approve' : 'request_review')} className={`mt-3 rounded-lg px-5 py-2.5 text-sm font-black text-white disabled:opacity-60 ${intent === 'approve' ? 'bg-emerald-600' : 'bg-amber-600'}`}>{busyId ? 'Submitting…' : intent === 'approve' ? 'Confirm Approval' : 'Confirm Dispute'}</button></section> : null}
      <div className="space-y-2 sm:hidden">{displayedTimesheets.map((item) => { const start = item.weekStartDate ?? item.workDate; const regular = Math.min(item.totalHours, 40); const overtime = Math.max(item.totalHours - 40, 0); return <article key={item.id} className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h2 className="truncate text-sm font-black text-slate-950">{item.employeeName}</h2><p className="mt-0.5 truncate text-xs text-slate-500">{item.jobSiteName}</p></div><div className="shrink-0 text-center"><p className="min-h-5 text-xl font-black leading-none text-slate-950">{item.totalHours ? item.totalHours.toFixed(1) : ''}</p><p className="mt-1 text-[10px] font-bold uppercase tracking-wide text-slate-400">Total hrs</p></div></div><div className="mt-3 grid grid-cols-7 gap-1">{WEEK_DAYS.map((day, index) => { const value = hoursForDate(item, dateAt(start, index)); return <div key={day} className="rounded-md bg-slate-100 px-0.5 py-1.5 text-center"><span className="block text-[9px] font-bold uppercase text-slate-500">{day.slice(0, 2)}</span><span className="mt-0.5 block min-h-4 text-sm font-black text-slate-900">{value ? value.toFixed(value % 1 ? 1 : 0) : ''}</span></div>; })}</div><div className="mt-2 flex items-center justify-between text-[11px]"><span className={item.hasSignedTimesheet ? 'font-bold text-slate-500' : 'font-bold text-red-600'}>{item.hasSignedTimesheet ? '✓ Signed copy attached' : '⚠ No signed copy'}</span><span className="text-slate-500">Regular {regular.toFixed(1)}{overtime ? ` · OT ${overtime.toFixed(1)}` : ''}</span></div>{!intent ? <div className="mt-3">{item.approvedAt ? <div className="rounded-lg bg-emerald-50 py-2 text-center text-xs font-black text-emerald-700">✓ Approved</div> : item.reviewRequestedAt ? <div className="rounded-lg bg-amber-50 py-2 text-center text-xs font-black text-amber-800">Changes requested</div> : <div className="grid grid-cols-2 gap-2"><button disabled={Boolean(busyId)} onClick={() => void decide(item.id, 'approve')} className="min-h-10 rounded-lg bg-emerald-600 px-3 text-xs font-black text-white disabled:opacity-60">Approve</button><button disabled={Boolean(busyId)} onClick={() => setReviewingId(item.id)} className="min-h-10 rounded-lg border border-amber-300 bg-amber-50 px-3 text-xs font-black text-amber-800 disabled:opacity-60">Request changes</button></div>}</div> : null}</article>; })}</div>
      <div className="hidden overflow-x-auto rounded-xl border border-slate-300 [touch-action:pan-x] sm:block">
        <table className="w-full min-w-[1320px] border-collapse text-xs">
          <thead className="bg-black text-white"><tr><th colSpan={14} className="px-3 py-3 text-center text-xl font-black">Mc Labor Sources, Inc. Timesheets</th></tr><tr>
            ...
            <td className="border border-slate-300 px-2 py-2"><p className={item.hasSignedTimesheet ? 'font-bold text-slate-700' : 'font-bold text-red-600'}>{item.hasSignedTimesheet ? 'Signed Timesheet attached' : 'No Timesheet'}</p>{!intent ? <div className="mt-2 flex gap-2">{item.approvedAt ? <span className="font-black text-emerald-700">✓ Approved</span> : item.reviewRequestedAt ? <span className="font-black text-amber-800">Changes Requested</span> : <><button disabled={Boolean(busyId)} onClick={() => void decide(item.id, 'approve')} className="min-h-10 rounded bg-emerald-600 px-3 font-black text-white disabled:opacity-60">Approve</button><button disabled={Boolean(busyId)} onClick={() => setReviewingId(item.id)} className="min-h-10 rounded border border-amber-400 bg-amber-50 px-3 font-black text-amber-800 disabled:opacity-60">Request Changes</button></>}</div> : null}</td>
          </tr>; })}</tbody>
        </table>
      </div>
