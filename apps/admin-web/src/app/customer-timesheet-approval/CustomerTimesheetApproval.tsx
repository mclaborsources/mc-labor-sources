...
export function CustomerTimesheetApproval({ token, approveAll = false, demo = false }: { token: string; approveAll?: boolean; demo?: boolean }) {
  ...
  useEffect(() => {
    if (reviewComplete && !loading && !busyId) window.close();
  }, [reviewComplete, loading, busyId]);

  // Browsers may refuse to close tabs opened directly from an email link.
  if (reviewComplete && !loading && !busyId) return <main className="flex min-h-screen items-center justify-center bg-slate-100 p-6">
    <div role="status" className="max-w-lg rounded-2xl border border-emerald-200 bg-white p-8 text-center shadow-lg">
      <h1 className="text-2xl font-bold text-emerald-800">Review complete</h1>
      <p className="mt-3 text-lg text-slate-700">All timesheets have been approved or marked for changes. You can close this tab.</p>
    </div>
  </main>;
