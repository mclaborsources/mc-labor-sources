...
export function NextWeekPreviewAccess({ employeeId }: { employeeId: string }) {
  ...
  return <div className="grid grid-cols-2 sm:grid-cols-[minmax(0,1fr)_7.5rem_7.5rem] [&>div:first-child]:col-span-2 sm:[&>div:first-child]:col-span-1 items-center gap-x-3 gap-y-2 border-t border-slate-200 px-3 py-2">
    <div>
      <div className="flex flex-wrap items-center gap-2"><p className="text-xl leading-6 font-semibold text-slate-900">Next work week (NW) · Automatic</p>
        <button type="button" onClick={() => setRules(true)} className="rounded-lg border border-slate-200 bg-white px-2 py-1 text-base font-semibold">View Rules</button>
      </div>
      <p className={`mt-1 text-lg leading-6 font-semibold ${nextWeekEnabled ? 'text-emerald-700' : 'text-slate-600'}`}>
      ...
      {nextWeekEnabled && data ? <p className="mt-1 text-lg leading-6 text-slate-600">Expires {new Date(data.expiresAt).toLocaleString('en-US', { timeZone: 'America/New_York' })} Eastern Time</p> : null}
    </div>
    <button type="button" disabled={checking || override.isPending || nextWeekEnabled} onClick={() => override.mutate(true)}
      className={`rounded-lg min-h-11 px-2 py-2 text-lg leading-6 font-bold ${!checking && nextWeekEnabled ? 'bg-emerald-600 text-white ring-2 ring-emerald-200' : 'border border-slate-300 bg-white text-slate-600'}`}>
    ...
    <button type="button" disabled={checking || override.isPending || !nextWeekEnabled} onClick={() => override.mutate(false)}
      className={`rounded-lg min-h-11 px-2 py-2 text-lg leading-6 font-bold ${!checking && !nextWeekEnabled ? 'bg-red-600 text-white ring-2 ring-red-200' : 'border border-slate-300 bg-white text-slate-600'}`}>
