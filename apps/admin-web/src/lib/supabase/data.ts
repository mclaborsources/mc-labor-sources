...
function mapEmployee(row: Record<string, unknown>): Employee {
  return {
    ...
    mobileNextWeekEnabled: Boolean(row.mobile_next_week_enabled),
...
export const data = {
  ...
  async updateEmployee(id: string, payload: Partial<Employee>): Promise<Employee> {
    ...
    if (payload.mobileNextWeekEnabled !== undefined) update.mobile_next_week_enabled = payload.mobileNextWeekEnabled;
  ...
  async previewSignedTimesheet(timesheetId: string): Promise<Blob> {
    ...
    const response = await fetch(endpoint, {
      ...
      body: JSON.stringify({ action: 'preview', timesheetId }),
    });
    if (!response.ok) {
      const result = await response.json().catch(() => ({ error: 'Could not create PDF preview' }));
      throw new DataError(result.error || 'Could not create PDF preview');
    }
