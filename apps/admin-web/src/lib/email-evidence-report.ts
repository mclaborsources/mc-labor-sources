...
export function buildEmailBatchReport(input: {
  ...
  const relevant = decisions
    ...
    .filter((decision) => decision.source_batch_id === batch.id || decision.source_batch_id === null)
    .sort((left, right) => left.decided_at.localeCompare(right.decided_at));
  const linkedCount = relevant.filter((decision) => decision.source_batch_id === batch.id).length;
  ...
  const rows = items.map((item) => {
    const linked = relevant.filter((decision) =>
      decision.timesheet_id === item.timesheet_id && decision.source_batch_id === batch.id);
    ...
    const status = latest?.decision === 'APPROVED' ? 'Approved from this email'
      : latest?.decision === 'CHANGES_REQUESTED' ? 'Changes requested from this email'
        : historical.length ? 'Historical action; email link unknown' : 'No action linked to this email';
    ...
    const action = (decision: EvidenceDecision) => {
      const verifiedLink = decision.source_batch_id === batch.id;
      return `<li><strong>${decision.decision === 'APPROVED' ? 'Approved hours' : 'Requested changes'}</strong> · ${date(decision.decided_at)}<span class="source">${verifiedLink ? 'Action linked to this sent email' : 'Historical action; source email was not recorded'}</span>${decision.comment ? `<p class="comment">${html(decision.comment)}</p>` : ''}</li>`;
    };
    return `<article class="timesheet"><div class="timesheet-heading"><div><h3>${html(name(item))}</h3><p>Week ending ${html(item.timesheet?.week_end_date ?? 'date unavailable')} · ${html(String(item.timesheet?.total_hours ?? 0))} hours</p></div><span class="status ${tone}">${status}</span></div>${linked.length || historical.length ? `<ol class="actions">${[...linked, ...historical].sort((left, right) => left.decided_at.localeCompare(right.decided_at)).map(action).join('')}</ol>` : '<p class="empty">No customer portal action recorded for this timesheet.</p>'}<p class="id">Timesheet ID: ${html(item.timesheet_id)}</p></article>`;
  }).join('');
