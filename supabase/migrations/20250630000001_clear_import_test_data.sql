...
BEGIN
  ...
  WITH deleted AS (DELETE FROM attendance_logs RETURNING 1)
  SELECT count(*)::int INTO v_attendance FROM deleted;

  WITH deleted AS (DELETE FROM timesheets RETURNING 1)
  SELECT count(*)::int INTO v_timesheets FROM deleted;

  WITH deleted AS (DELETE FROM job_orders RETURNING 1)
  SELECT count(*)::int INTO v_job_orders FROM deleted;

  WITH deleted AS (DELETE FROM job_assignments RETURNING 1)
  SELECT count(*)::int INTO v_assignments FROM deleted;

  WITH deleted AS (DELETE FROM supervisor_job_sites RETURNING 1)
  SELECT count(*)::int INTO v_supervisor_links FROM deleted;

  DELETE FROM safety_bulletin_recipients;
  WITH deleted AS (DELETE FROM safety_bulletins RETURNING 1)
  SELECT count(*)::int INTO v_safety_bulletins FROM deleted;

  WITH deleted AS (DELETE FROM job_sites RETURNING 1)
  ...
  WITH deleted AS (DELETE FROM customers RETURNING 1)
  SELECT count(*)::int INTO v_customers FROM deleted;

  WITH deleted AS (DELETE FROM employees RETURNING 1)
  SELECT count(*)::int INTO v_employees FROM deleted;

  WITH deleted AS (DELETE FROM data_import_runs RETURNING 1)
  SELECT count(*)::int INTO v_import_runs FROM deleted;

  WITH deleted AS (DELETE FROM email_delivery_log RETURNING 1)
